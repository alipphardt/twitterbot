...
class TwitterBot:
    ...
    def __init__(self, twitter_consumer_key, twitter_consumer_secret, 
                 ...
                 bitly_access_token='', bitly_account='', 
                 news_api_key=''):
        ...
        self.__bitly_account = bitly_account
    ...
    def news(self):
        ...
        response = requests.get("https://newsapi.org/v2/everything?q=" + q + "&from=" + datetime.now().strftime("%Y-%m-%d") + "&sortBy=popularity&pageSize=100&apiKey=" + self.__news_api_key)
        ...
        if self.__bitly_access_token != '':
            ...
            for index, article in news_api_articles.iterrows():
                url = article['url']
                bitly_response = requests.get("https://api-ssl.bitly.com/v3/shorten", params = {'longUrl':url,'access_token': self.__bitly_access_token})
    ...
    def twitter(self):
        ...
        self.list = pd.DataFrame(tweets, columns=['id','title', 'user', 'publishedAt','followers_count']).nlargest(10,'followers_count')
            ...
            for index, item in self.list.iterrows():
                ...
                try:
                    if self.__image == None:
                        ...
                        self.__api.update_with_media(filename=self.__image, status=message, in_reply_to_status_id=item['id'])
                except KeyError:
